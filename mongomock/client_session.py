...
from pymongo.errors import ConnectionFailure, WTimeoutError  # FIXME
from pymongo.helpers import _RETRYABLE_ERROR_CODES  # FIXME

from mongomock import (ConfigurationError, OperationFailure, InvalidOperation,
                       PyMongoError)
from mongomock.collection import ReadConcern, WriteConcern, ReadPreference
...
class TransactionOptions(object):
    ...
    def __init__(self, read_concern=None, write_concern=None,
        ...
        if read_concern is not None:
            if not isinstance(read_concern, ReadConcern):
                raise TypeError("read_concern must be an instance of "
                                "mongomock.collection.ReadConcern, not: %r" %
                                (read_concern,))
        if write_concern is not None:
            if not isinstance(write_concern, WriteConcern):
                raise TypeError("write_concern must be an instance of "
                                "mongomock.collection.WriteConcern, not: %r" %
                                (write_concern,))
        ...
        if read_preference is not None:
            if not isinstance(read_preference, ReadPreference):
...
class ClientSession(object):
    ...
    def _finish_transaction(self, sock_info, command_name):
        ...
        cmd = OrderedDict([(command_name, 1)])  # FIXME SON([(command_name,1)])
    ...
    def advance_cluster_time(self, cluster_time):
        ...
        # FIXME if not isinstance(cluster_time.get("clusterTime"), Timestamp):
        if not isinstance(cluster_time.get("clusterTime"), int):
    ...
    def advance_operation_time(self, operation_time):
        ...
        :Parameters:
          - `operation_time`: The
            ...
            another `ClientSession` instance.
        """
        # FIXME if not isinstance(operation_time, Timestamp):
        if not isinstance(operation_time, int):
    ...
    def __copy__(self):
        raise TypeError('A ClientSession cannot be copied, create'
...
class _ServerSession(object):
    ...
    def __init__(self, generation):
        ...
        sess_id = {'id': uuid.uuid4()}  # FIXME Binary(uuid.uuid4().bytes, 4)
    ...
    def transaction_id(self):
        return self._transaction_id  # FIXME Int64(self._transaction_id)
...
SESSION_POOL = _ServerSessionPool()
